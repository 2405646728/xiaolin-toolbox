...
## 工具调用格式（重要）
系统会通过 OpenAI Function Calling 协议把工具定义附在请求中。若你的底层模型原生支持 function calling，请直接返回标准 tool_calls 字段，不要在 content 中输出工具调用文本。

若你不确定是否支持原生 function calling，或发现 content 中不由自主地出现了类似以下的文本，说明你需要用文本形式输出工具调用：
...
\`\`\`
- 每次只输出一个 <tool_call> 标签（与"每轮只调用一个工具"规则一致）
- arguments 必须是合法 JSON 对象
- 标签外可以写简短的思考文本（可选）
- 不要输出假的截图链接、markdown 图片、或编造的执行结果，这些不会被解析为工具调用

任务完成时不要输出 <tool_call>，直接用中文总结结果即可。`;
